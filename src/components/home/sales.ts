...
export function Sales(ctx: PageContext): string {
  const { e } = ctx.s;
  return `    <section id="keep-watching" class="flow" data-space="s" aria-labelledby="keep-watching-heading">
      <h2 id="keep-watching-heading" class="text-2xl">${e('sales.title')}</h2>
      <p class="text-lg text-muted max-w-prose">${e('sales.body')}</p>
      <div class="grid" data-min="s" data-align="stretch">
        <article class="card flow" data-space="xs">
          ...
          <a class="button" data-variant="primary" href="${APP}/signup/">${e('sales.free.cta')}</a>
...
susbot https://example.com --fail-on warning</code></pre>
          ...
          <a class="button" href="${LINKS.engine}#readme">${e('sales.self.cta')}</a>
        </article>
        <article class="card flow" data-space="xs">
          ...
          <a class="button" href="${APP}/signup/">${e('sales.watch.cta')}</a>
        </article>
      </div>
    </section>
`;
